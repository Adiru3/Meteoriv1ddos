...
def proxyget(url): # scarica proxy da altri siti
	try:
		...
		print("Proizoshla oshibka, perekhod na sleduyushchiy sayt.\n") # printa questo
...
def proxylist(): # funzione per la creazione della proxylist
	...
	print("Tekushchiye IP-adresa v proksiliste: %s" % (len(open("proxy.txt").readlines()))) # per vedere quante lines (e quindi quanti proxy) ci sono nel file
	print ("\Obnovlen nProxylist!\n")
...
def main(): # funzione effettiva del programma.
	try:
		...
		print ("\nIdet zagruzka iz spiska svobodnykh proksi ...")
		...
		for position, url in enumerate(nurls):
			proxyget(url)
			print("Zavershennyye zagruzki: (%s/%s)\nTekushchiye IP-adresa v proksiliste:: %s" % (position+1, len(nurls), len(open("proxy.txt").readlines())))
		...
		print("\n")
		while True:
			choice = input("\nKhotite proverit' proksi? [Y/n] > ") # scelta di quello che vuole l'utente
			...
			if choice == 'N' or choice == 'n' or choice == 'no' or choice == 'No': # altrimenti esce
				...
				print ("Pozhaluysta, napishite pravil'no.")
